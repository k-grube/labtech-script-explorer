/**
 * Created by kgrube on 2/9/2018
 */
import React, {Component} from 'react';
import PropTypes from 'prop-types';
import {connect} from 'react-redux';
import JsonTree from 'react-json-tree';
import {setJSON} from '../../redux/script';

@connect(state => ({
  scriptJSON: state.script.scriptJSON,
  LabTechScript: state.script.LabTechScript,
  /* eslint-disable indent */
}), {setJSON})
class EditorJSON extends Component {
  shouldExpandNode = (keyName, data, level) => {
    return level < 6;
  };

  render() {
    const theme = {
      scheme: 'monokai',
      author: 'wimer hazenberg (http://www.monokai.nl)',
      base00: '#272822',
      base01: '#383830',
      base02: '#49483e',
      base03: '#75715e',
      base04: '#a59f85',
      base05: '#f8f8f2',
      base06: '#f5f4f1',
      base07: '#f9f8f5',
      base08: '#f92672',
      base09: '#fd971f',
      base0A: '#f4bf75',
      base0B: '#a6e22e',
      base0C: '#a1efe4',
      base0D: '#66d9ef',
      base0E: '#ae81ff',
      base0F: '#cc6633',
    };

    return (
      <JsonTree
        data={this.props.LabTechScript}
        theme={theme}
        invertTheme
        hideRoot
        shouldExpandNode={this.shouldExpandNode}
      />
    );
  }
}

EditorJSON.defaultProps = {
  LabTechScript: undefined,
};

/* eslint-disable react/no-unused-prop-types */
EditorJSON.propTypes = {
  scriptJSON: PropTypes.string.isRequired,
  setJSON: PropTypes.func.isRequired,
  LabTechScript: PropTypes.object,
};

export default EditorJSON;
